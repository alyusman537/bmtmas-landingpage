...
export default function LandingClient({
  produkSimpanan: initialSimpanan,
  produkPembiayaan: initialPembiayaan,
  beritaData: initialBerita,
  galeriData: initialGaleri,
}) {
  ...
  const [produkSimpanan] = useState(initialSimpanan);
  const [produkPembiayaan] = useState(initialPembiayaan);
  const [beritaData] = useState(initialBerita);
  const [galeriData] = useState(initialGaleri);
                      ...
                      <div className="relative h-40 overflow-hidden">
                        ...
                        <div className="absolute bottom-0 left-0 right-0 p-5 space-y-2">
                          ...
                          <h3 className="text-lg font-bold text-white leading-snug">
                            <Link href={"/berita/" + item.id} className="hover:underline">{item.judul}</Link>
                          </h3>
                        </div>
                      </div>
                    ) : (
              ...
              <div>
                ...
                <ul className="space-y-2.5 text-[var(--text-secondary)] text-sm md:text-base">
                  <li className="flex items-start gap-2">
                    ...
                    Mengelola Koperasi dan unit usaha secara profesional dengan menerapkan prinsip &ldquo;Good Corporate Governance&rdquo; untuk menciptakan kesejahteraan anggota.
                  </li>
                  <li className="flex items-start gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-[var(--kop-green)] shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>
          ...
          <form onSubmit={handleSubmit} className="p-8 md:col-span-3 space-y-4 bg-[var(--bg-card)]">
            <div>
              ...
              <input
                type="text"
                required
                className="w-full px-4 py-2 border border-[var(--border-medium)] rounded-lg focus:outline-none focus:border-[var(--kop-blue)] bg-[var(--bg-page)] text-[var(--text-primary)] text-sm"
                value={formData.nama}
            ...
            <div>
              ...
              <textarea
                rows="3"
                required
                className="w-full px-4 py-2 border border-[var(--border-medium)] rounded-lg focus:outline-none focus:border-[var(--kop-blue)] bg-[var(--bg-page)] text-[var(--text-primary)] text-sm resize-none"
