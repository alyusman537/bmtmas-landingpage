...
router.post('/', auth, async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Gagal menambah produk simpanan' });
  }
});
...
router.put('/:id', auth, async (req, res) => {
  try {
    ...
    const [rows] = await pool.query('SELECT * FROM simpanan WHERE id = ?', [req.params.id]);
    ...
    res.status(500).json({ message: 'Gagal mengupdate produk simpanan' });
  }
});

router.delete('/:id', auth, async (req, res) => {
  try {
    const [result] = await pool.query('DELETE FROM simpanan WHERE id = ?', [req.params.id]);
    ...
    res.status(500).json({ message: 'Gagal menghapus produk simpanan' });
  }
});
